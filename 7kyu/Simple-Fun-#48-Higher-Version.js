...
function higherVersion(ver1, ver2) {
    const arrForVer1 = ver1.split('.').map(Number);
    const arrForVer2 = ver2.split('.').map(Number);

    let result = false;

    for (let i = 0; i < arrForVer1.length; i++) {

        if(arrForVer1[i]>arrForVer2[i]) {
            result = true;
            break;
        } else if(arrForVer2[i]>arrForVer1[i]){
            result = false;
            break;
        }
    }
